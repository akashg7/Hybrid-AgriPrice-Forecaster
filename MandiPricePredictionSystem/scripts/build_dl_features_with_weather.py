...
OUTPUT_FILE = Path("data/processed/dl_advanced_features_data.csv")
...
def main():
    ...
    # Because of lag 60, we might lose 60 rows per group if we strictly drop.
    # Let's fill backwards slightly or drop safely.
    df_final = df_final.dropna(subset=["target_price", "modal_lag_14"]).fillna(0) # Forward fill rest as fallback
